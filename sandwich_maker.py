...
class Sandwich(db.Model):
    ...
    id_column = db.Column(db.Integer, primary_key=True)
...
@app.route("/sandwich")
def sandwich():
    return render_template("sandwiches/list.html", resources=Sandwich.query.all())
...
@app.route('/updatesandwich/<int:id_column>/', methods=['GET', 'POST'])
def update_sandwich(id_column):
    if request.method == 'POST':
        if not request.form['sandwich_size'] or not request.form['price']:
            flash('Please enter all the fields', 'error')
        else:
            sandwich = Sandwich.query.filter_by(id=id_column).first()
            ...
            flash('Record was successfully updated')
            return redirect(url_for('sandwich'))
    data = Sandwich.query.filter_by(id=id_column).first()
    return render_template("sandwiches/update.html", data=data)


@app.route('/deletesandwich/<int:id_column>/', methods=['GET', 'POST'])
def delete_sandwich(id_column):
    if request.method == 'POST':
        sandwich = Sandwich.query.filter_by(id=id_column).first()
